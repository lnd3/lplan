class ItemsView {
  ...
  static async show() {
    ...
    const statusContainer = document.getElementById('items-view');
    statusContainer.style.display = 'block';

    await ItemsView.loadStatus();
    ItemsView.render();
  }
  ...
  static async loadStatus() {
    try {
      ...
      ItemsView.data = result.data.entities;
      ItemsView.applyFilters();
    } catch (e) {
  ...
  static render() {
    ...
    if (!container.querySelector('.status-filter-bar')) {
      ...
      html += ItemsView.renderFilterBar();
      ...
      ItemsView.attachEventListeners();
    }

    const resultsContainer = container.querySelector('.status-results-container');
    let html = ItemsView.renderTable() + ItemsView.renderPagination();
    resultsContainer.innerHTML = html;
    ItemsView.attachTableEventListeners();
  }

  static renderFilterBar() {
    const resultCount = ItemsView.filteredData?.length || 0;
    const activeFilters = (ItemsView.filters.search ? 1 : 0) +
                         ItemsView.filters.statuses.length +
                         ItemsView.filters.priorities.length +
                         ItemsView.filters.types.length;
    ...
    html += `<input type="text" id="status-search" placeholder="🔍 Search ID, title, description..."
      style="padding: 6px 12px; background: #0f111b; border: 1px solid #313244; border-radius: 4px;
             color: #cdd6f4; font-size: 13px; flex: 1; min-width: 200px;" value="${ItemsView.filters.search}">`;
    ...
    html += `<select id="status-filter" multiple style="padding: 6px 8px; background: #313244;
      ...
      <option value="IDEA" ${ItemsView.filters.statuses.includes('IDEA') ? 'selected' : ''}>IDEA</option>
      <option value="PLANNING" ${ItemsView.filters.statuses.includes('PLANNING') ? 'selected' : ''}>PLANNING</option>
      <option value="IN_PROGRESS" ${ItemsView.filters.statuses.includes('IN_PROGRESS') ? 'selected' : ''}>IN_PROGRESS</option>
      <option value="BLOCKED" ${ItemsView.filters.statuses.includes('BLOCKED') ? 'selected' : ''}>BLOCKED</option>
      <option value="DONE" ${ItemsView.filters.statuses.includes('DONE') ? 'selected' : ''}>DONE</option>
      <option value="DEFERRED" ${ItemsView.filters.statuses.includes('DEFERRED') ? 'selected' : ''}>DEFERRED</option>
      <option value="CANCELLED" ${ItemsView.filters.statuses.includes('CANCELLED') ? 'selected' : ''}>CANCELLED</option>
      </optgroup>
      <optgroup label="Thesis">
      <option value="HELD" ${ItemsView.filters.statuses.includes('HELD') ? 'selected' : ''}>HELD</option>
      <option value="QUESTIONING" ${ItemsView.filters.statuses.includes('QUESTIONING') ? 'selected' : ''}>QUESTIONING</option>
      <option value="ABANDONED" ${ItemsView.filters.statuses.includes('ABANDONED') ? 'selected' : ''}>ABANDONED</option>
      </optgroup>
      <optgroup label="Concept">
      <option value="STABLE" ${ItemsView.filters.statuses.includes('STABLE') ? 'selected' : ''}>STABLE</option>
      <option value="DRAFT" ${ItemsView.filters.statuses.includes('DRAFT') ? 'selected' : ''}>DRAFT</option>
      <option value="DEPRECATED" ${ItemsView.filters.statuses.includes('DEPRECATED') ? 'selected' : ''}>DEPRECATED</option>
      </optgroup>
    </select>`;

    html += `<select id="priority-filter" multiple style="padding: 6px 8px; background: #313244;
             border: 1px solid #45475a; border-radius: 4px; color: #cdd6f4; font-size: 12px; min-height: 24px;">
      <option value="HIGH" ${ItemsView.filters.priorities.includes('HIGH') ? 'selected' : ''}>HIGH</option>
      <option value="MEDIUM" ${ItemsView.filters.priorities.includes('MEDIUM') ? 'selected' : ''}>MEDIUM</option>
      <option value="LOW" ${ItemsView.filters.priorities.includes('LOW') ? 'selected' : ''}>LOW</option>
    </select>`;

    html += `<select id="type-filter" multiple style="padding: 6px 8px; background: #313244;
             border: 1px solid #45475a; border-radius: 4px; color: #cdd6f4; font-size: 12px; min-height: 24px;">
      <option value="concept" ${ItemsView.filters.types.includes('concept') ? 'selected' : ''}>Concept</option>
      <option value="thesis" ${ItemsView.filters.types.includes('thesis') ? 'selected' : ''}>Thesis</option>
      <option value="master_plan" ${ItemsView.filters.types.includes('master_plan') ? 'selected' : ''}>Master Plan</option>
    ...
    html += `<span style="font-size: 12px; color: #a6adc8; margin-left: auto;">
      Showing ${Math.min((ItemsView.currentPage - 1) * ItemsView.pageSize + 1, resultCount)}-${Math.min(ItemsView.currentPage * ItemsView.pageSize, resultCount)} of ${resultCount}
    </span>`;
  ...
  static renderTable() {
    if (!ItemsView.filteredData || ItemsView.filteredData.length === 0) {
      return '<div style="color: #a6adc8; padding: 40px; text-align: center;">No entities match your filters</div>';
    }

    const start = (ItemsView.currentPage - 1) * ItemsView.pageSize;
    const end = start + ItemsView.pageSize;
    const pageData = ItemsView.filteredData.slice(start, end);
    ...
    for (const col of columns) {
      ...
      const isSorted = ItemsView.sortConfig.column === col;
      const arrow = isSorted ? (ItemsView.sortConfig.direction === 'asc' ? ' ↑' : ' ↓') : '';
    ...
    for (const entity of pageData) {
      const statusColor = ItemsView.getStatusColor(entity.status);
      const typeIcon = ItemsView.getTypeIcon(entity.type);
      const typeColor = ItemsView.getTypeColor(entity.type);

      // Parent badges + conviction
      let extraBadges = '';
      if (entity.type === 'concept') {
        extraBadges = ItemsView.badge(entity.concept_type || '?', '#94e2d5');
      } else if (entity.type === 'thesis' && entity.conviction) {
        extraBadges = ItemsView.badge(entity.conviction, '#cba6f7', true);
      } else if (entity.type === 'master_plan') {
        extraBadges = ItemsView.parentBadges(entity.parent_thesis, '#cba6f7');
      } else if (entity.type === 'project') {
        extraBadges = ItemsView.parentBadges(entity.parent_master_plan, '#f9e2af');
      ...
      html += `<tr style="border-bottom: 1px solid #313244; cursor: pointer;" data-id="${entity.id}" data-type="${entity.type}">
        ...
        <td style="padding: 8px 12px; color: #a6adc8;">${ItemsView.renderCompletion(entity)}</td>
        <td style="padding: 8px 12px; color: #9399b2; font-size: 12px;">${ItemsView.formatDate(entity.created)}</td>
        <td style="padding: 8px 12px; color: #9399b2; font-size: 12px;">${ItemsView.formatDate(entity.updated)}</td>
  ...
  static renderPagination() {
    const totalPages = Math.ceil((ItemsView.filteredData?.length || 0) / ItemsView.pageSize);
    if (totalPages <= 1) return '';

    let html = '<div style="margin-top: 16px; display: flex; gap: 8px; align-items: center; justify-content: center;">';

    if (ItemsView.currentPage > 1) {
      html += `<button class="pagination-btn" data-page="${ItemsView.currentPage - 1}" style="padding: 4px 8px; background: #313244; border: 1px solid #45475a; color: #cdd6f4; cursor: pointer; border-radius: 3px;">← Prev</button>`;
    }

    for (let i = 1; i <= Math.min(totalPages, 5); i++) {
      const isActive = i === ItemsView.currentPage;
    ...
    if (ItemsView.currentPage < totalPages) {
      html += `<button class="pagination-btn" data-page="${ItemsView.currentPage + 1}" style="padding: 4px 8px; background: #313244; border: 1px solid #45475a; color: #cdd6f4; cursor: pointer; border-radius: 3px;">Next →</button>`;
    }
  ...
  static attachEventListeners() {
    ...
    if (searchInput && !searchInput._attached) {
      ...
      searchInput.addEventListener('input', (e) => {
        ItemsView.filters.search = e.target.value;
        ItemsView.currentPage = 1;
        ItemsView.applyFilters();
        ItemsView.render();
      });
    }
    ...
    if (statusFilter && !statusFilter._attached) {
      ...
      statusFilter.addEventListener('change', (e) => {
        ItemsView.filters.statuses = Array.from(e.target.selectedOptions).map(o => o.value).filter(v => v);
        ItemsView.currentPage = 1;
        ItemsView.applyFilters();
        ItemsView.render();
      });
    ...
    if (priorityFilter && !priorityFilter._attached) {
      ...
      priorityFilter.addEventListener('change', (e) => {
        ItemsView.filters.priorities = Array.from(e.target.selectedOptions).map(o => o.value).filter(v => v);
        ItemsView.currentPage = 1;
        ItemsView.applyFilters();
        ItemsView.render();
      });
    ...
    if (typeFilter && !typeFilter._attached) {
      ...
      typeFilter.addEventListener('change', (e) => {
        ItemsView.filters.types = Array.from(e.target.selectedOptions).map(o => o.value).filter(v => v);
        ItemsView.currentPage = 1;
        ItemsView.applyFilters();
        ItemsView.render();
      });
    ...
    if (clearBtn && !clearBtn._attached) {
      ...
      clearBtn.addEventListener('click', () => {
        ItemsView.filters = { search: '', statuses: [], priorities: [], types: [] };
        ItemsView.currentPage = 1;
  ...
  static attachTableEventListeners() {
    ...
    document.querySelectorAll('th[data-column]').forEach(th => {
      th.addEventListener('click', () => {
        ...
        if (ItemsView.sortConfig.column === column) {
          ItemsView.sortConfig.direction = ItemsView.sortConfig.direction === 'asc' ? 'desc' : 'asc';
        } else {
          ItemsView.sortConfig.column = column;
          ItemsView.sortConfig.direction = 'asc';
        }
        ItemsView.applyFilters();
        ItemsView.render();
      });
    ...
    document.querySelectorAll('.pagination-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ItemsView.currentPage = parseInt(btn.dataset.page);
        ItemsView.render();
      });
    });
    ...
    document.querySelectorAll('tr[data-id]').forEach(row => {
      row.addEventListener('click', () => {
        ...
        const entity = ItemsView.data.find(e => e.id === id && e.type === type);
        if (entity) {
  ...
  static applyFilters() {
    let filtered = [...(ItemsView.data || [])];

    if (ItemsView.filters.search) {
      const q = ItemsView.filters.search.toLowerCase();
    ...
    if (ItemsView.filters.statuses.length > 0) {
      filtered = filtered.filter(e => ItemsView.filters.statuses.includes(e.status));
    }

    if (ItemsView.filters.priorities.length > 0) {
      filtered = filtered.filter(e => ItemsView.filters.priorities.includes(e.priority));
    }

    if (ItemsView.filters.types.length > 0) {
      filtered = filtered.filter(e => ItemsView.filters.types.includes(e.type));
    }

    ItemsView.sortData(filtered);
    ItemsView.filteredData = filtered;
  }

  static sortData(data) {
    const col = ItemsView.sortConfig.column;
    const dir = ItemsView.sortConfig.direction;
  ...
  static getTypeColor(type) {
    return ItemsView.TYPE_COLORS[type] || '#cdd6f4';
  }
  ...
  static parentBadges(ids, color) {
    ...
    return ids.map(id => ItemsView.badge(id, color)).join('');
  }
