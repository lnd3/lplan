class StatusView {
  ...
  static goTo(id, title, type, path) {
    const resolvedPath = path || `${StatusView.TYPE_DIRS[type] || 'projects'}/${id}.md`;
  ...
  static render() {
    ...
    let html = StatusView.renderTotals(d.totals);
    html += StatusView.renderNeedsAttention(d.needs_attention, d.stale_days_threshold);
    html += StatusView.renderRollupSection('master-plans', 'Master Plans', d.master_plan_rollups, 'master_plan');
    html += StatusView.renderRollupSection('projects', 'Projects', d.project_rollups, 'project');
  ...
  static renderCollapsible(sectionId, headingHtml, bodyHtml) {
    const isCollapsed = StatusView.collapsed.has(sectionId);
  ...
  static renderNeedsAttention(na, staleDays) {
    ...
    if (na.stale.length) {
      let rows = na.stale.map(e => `<div style="padding: 6px 0; border-bottom: 1px solid #313244; display: flex; justify-content: space-between; cursor: pointer;" data-id="${e.id}" data-type="${e.type}" data-path="${e.path || ''}" data-title="${StatusView.escapeAttr(e.title)}">
    ...
    if (na.blocked.length) {
      let rows = na.blocked.map(e => `<div style="padding: 6px 0; border-bottom: 1px solid #313244; cursor: pointer;" data-id="${e.id}" data-type="${e.type}" data-path="${e.path || ''}" data-title="${StatusView.escapeAttr(e.title)}">
    ...
    if (warnings.length) {
      let rows = warnings.map(w => `<div style="padding: 6px 0; border-bottom: 1px solid #313244; cursor: pointer;" data-id="${w.id}" data-type="${w.type}" data-path="${w.path || ''}" data-title="${StatusView.escapeAttr(w.id)}">
        <span style="color: #cba6f7;">${w.id}</span> <span style="color: #a6adc8; font-size: 12px;">${StatusView.escapeAttr(w.message)}</span>
      </div>`).join('');
      sections.push(`<div class="needs-attention-section" style="margin-bottom: 12px;">
    ...
    if (!sections.length) {
      return StatusView.renderCollapsible('needs-attention', heading,
        `<div style="color: #a6e3a1; padding: 8px 0;">✓ Nothing needs attention — no stale, blocked, or reference issues found.</div>`);
    }

    return StatusView.renderCollapsible('needs-attention', heading, sections.join(''));
  }
  ...
  static renderRollupSection(sectionId, heading, rollups, type) {
    ...
    let rows = rollups.map(r => {
      ...
      return `<div style="padding: 8px 0; border-bottom: 1px solid #313244; cursor: pointer;" data-id="${r.id}" data-type="${type}" data-path="${r.path || ''}" data-title="${StatusView.escapeAttr(r.title)}">
    ...
    return StatusView.renderCollapsible(sectionId, headingHtml, rows);
  }

  static attachEventListeners() {
    document.querySelectorAll('#status-view [data-id][data-type]').forEach(el => {
      el.addEventListener('click', () => StatusView.goTo(el.dataset.id, el.dataset.title, el.dataset.type, el.dataset.path));
    });

    document.querySelectorAll('#status-view [data-section-toggle]').forEach(el => {
      el.addEventListener('click', () => {
        ...
        if (nowCollapsed) StatusView.collapsed.add(sectionId);
        else StatusView.collapsed.delete(sectionId);
      });
    });
  }
