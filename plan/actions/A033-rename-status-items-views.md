...
2026-08-30 — Regression found by the user: the P010 dashboard stopped scrolling. Root cause: the rename's file/id-reference sweep was scoped to `*.js`/`*.py` only and missed `style.css`, which still had `#overview-view { overflow-y: auto; }` — a rule now targeting an id that no longer exists, while the new `#status-view` (renamed from `#overview-view`) had no scroll rule at all. This is the same class of bug as A021's original scroll fix, self-inflicted by this rename. Fixed the one CSS selector; grepped `*.css` and `server.py` afterward for any other stale `overview-view`/`OverviewView` references — none found. Verified live via jsdom that `#status-view` now carries the `overflow-y:auto` rule again.
